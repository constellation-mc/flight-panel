...
import static dev.zenfyr.flightpanel.api.generators.Transformations.*;

import com.google.common.collect.Lists;
import dev.zenfyr.flightpanel.api.builders.ConfigScreenBuilder;
...
public class TestInit implements ClientModInitializer {
    ...
    ScreenEvents.BEFORE_INIT.register((client, screen, scaledWidth, scaledHeight) -> {
      if (screen.getClass() == OptionsScreen.class
        ...
        List<List<BaseElementBuilder<?, ?, ?>>> partition = Lists.partition(
            GuiRegistry.withDefaults()
                .generateForObject("test.flight-panel.config.", CONFIG, RealConfig::new),
            1);
  ...
  public static class RealConfig {
    ...
    public List<List<@Slider @Range(from = -3, to = 14) Integer>> nestedIntegers =
        List.of(List.of(3, 14), List.of(1));
