...
public class ListProviderFactory implements GuiProviderFactory {
    ...
    public record ListProvider<T>(GuiProvider<T, ? extends AbstractValuedElement<T, ?>, ?> elementAdapter) implements GuiProvider<List<T>, NestedListElement<T>, NestedListBuilder<T>> {
        ...
        @Override
        public @NotNull NestedListBuilder<T> provideGui(List<T> obj, Supplier<List<T>> def, ProviderContext context) {
            var entryCtx = context.withGeneric(true);
            return NestedListBuilder.create(context.i18nOrEmpty(), value(obj, def).orElse(Collections.emptyList()), (t, e) -> elementAdapter.provideGui(t, null, entryCtx).build(), () -> null).defaultValue(def);
        }
    }
}
