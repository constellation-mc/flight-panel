...
public class ListButtonBuilder<T> extends ListBasedBuilder<T, ListButtonElement<T>, ListButtonBuilder<T>> {
    ...
    public static <T extends Enum<T>> ListButtonBuilder<T> forEnum(Text elementName, T value, Function<T, Text> textifier) {
        return new ListButtonBuilder<>(elementName, value, Arrays.asList(value.getDeclaringClass().getEnumConstants()), textifier);
    }
